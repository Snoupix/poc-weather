...
async function getWeather(lat: WeatherProps["lat"], lon: WeatherProps["lon"], setWeatherState: React.Dispatch<React.SetStateAction<WeatherData | null>>) {
    try {
...
export default function Weather({ lat, lon, time }: WeatherProps) {
    ...
    return (
        <section className="xl:text-lg text-sm">
            <p>Current weather for your city: {weatherState.name} ({weatherState.sys.country})</p>
            <p>At: {time}</p>
            <p>Temp: {weatherState.main.temp}°C ({weatherState.main.temp_min}°C - {weatherState.main.temp_max}°C)</p>
            ...
            <div>-</div>
            {weatherState.weather.map(w => <p>{w.main} ({w.description})</p>)}
        </section>
    );
};
